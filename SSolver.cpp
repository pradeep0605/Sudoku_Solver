...
int main()
{
  ...
  /*void performGuarnteedRowColumnValueAnalysis() // To detect certain values of boxes which will appear only in one row or a column
   {
   		for(int b=0;b<9;b++)
		{
			int n= (b/3)*3;
			int m= (b%3)*3;
			//cout<<"Processing box "<<b<<" Starting at ("<<n<<","<<m<<")"<<endl;
			for(int i=0;i<9;i++)
				EnsuredBoxValues[i]=Box();
				
			Set checkrows[3],checkcolumns[3];
			for(int i=n;i< (n+3) ;i++)
				for(int j=m;j< (m+3); j++)
					if(cells[i][j].value==0)
						checkrows[i-n] = checkrows[i-n] + cells[i][j].PossibleValues;
					
			
			for(int i=n;i< (n+3) ;i++)
				for(int j=m;j< (m+3); j++)
					if(cells[i][j].value==0)
						checkcolumns[j-m] = checkcolumns[j-m] + cells[i][j].PossibleValues;
			
			Set temp1=Set(),temp2=Set(),temp3=Set();
			temp1=checkrows[0]; temp2=checkrows[1]; temp3=checkrows[2];
			
			Set temp=Set();
			temp = ((temp1 - temp2) - temp3);
			if( temp.getElementsCount() > 0)
			{
				int val;
				while(val=temp.getNextElement())
				{	EnsuredBoxValues[b].rows[0].addElement(val);	
					//cout<<val<<" was added to row 0 in the box "<<b<<endl; 
				}
			}
			...
			if( temp.getElementsCount() > 0)
			{
				...
				while(val=temp.getNextElement())
				{   
					EnsuredBoxValues[b].rows[1].addElement(val);	
					//cout<<val<<" was added to row 1 in the box "<<b<<endl; 
				}
			}
			
			temp =(( temp3 - temp1) - temp2);
			if( temp.getElementsCount() > 0)
			{
				int val;
				while(val=temp.getNextElement())
				{	
					EnsuredBoxValues[b].rows[2].addElement(val);	
					//cout<<val<<" was added to row 2 in the box "<<b<<endl; 
				}
			}
			
			//Column analysis starts
			
			temp1=Set(),temp2=Set(),temp3=Set();
			temp1=checkcolumns[0]; temp2=checkcolumns[1]; temp3=checkcolumns[2];
			...
			if( temp.getElementsCount() > 0)
			{
				...
				while(val=temp.getNextElement())
				{	
					EnsuredBoxValues[b].columns[0].addElement(val);	
					//cout<<val<<" was added to column 0 in the box "<<b<<endl; 
				}
			}
			...
			if( temp.getElementsCount() > 0)
			{
				...
				while(val=temp.getNextElement())
				{   
					EnsuredBoxValues[b].columns[1].addElement(val);	
					//cout<<val<<" was added to column 1 in the box "<<b<<endl; 
				}
			}
			...
			if( temp.getElementsCount() > 0)
			{
				...
				while(val=temp.getNextElement())
				{	
					EnsuredBoxValues[b].columns[2].addElement(val);	
					//cout<<val<<" was added to column 2 in the box "<<b<<endl; 
				}
			}
		}
   }
   	...
   	Set ensuredValues(int i,int j)
	{
		int b=getBoxValue(i,j);
		int I=i-((i/3)*3);
		int J= j - ((j/3)*3);
		//cout<<"In for the cell ("<<i<<","<<j<<") with I & J "<<I<<" "<<J<<endl;
		Set temp=Set();
		switch(b)
		{
			case 0: temp = temp + EnsuredBoxValues[1].rows[I]    + EnsuredBoxValues[2].rows[I];
				   temp = temp + EnsuredBoxValues[3].columns[J] + EnsuredBoxValues[6].columns[J];
				   break;
				   
			case 1: temp = temp + EnsuredBoxValues[0].rows[I]    + EnsuredBoxValues[2].rows[I];
				   temp = temp + EnsuredBoxValues[4].columns[J] + EnsuredBoxValues[7].columns[J];
				   break;
		
			case 2: temp = temp + EnsuredBoxValues[0].rows[I]    + EnsuredBoxValues[1].rows[I];
				   temp = temp + EnsuredBoxValues[5].columns[J] + EnsuredBoxValues[8].columns[J];
				   break;
				   
			case 3: temp = temp + EnsuredBoxValues[4].rows[I]    + EnsuredBoxValues[5].rows[I];
				   temp = temp + EnsuredBoxValues[0].columns[J] + EnsuredBoxValues[6].columns[J];
				   break;
			
			case 4: temp = temp + EnsuredBoxValues[3].rows[I]    + EnsuredBoxValues[5].rows[I];
				   temp = temp + EnsuredBoxValues[1].columns[J] + EnsuredBoxValues[7].columns[J];
				   break;
			
			case 5: temp = temp + EnsuredBoxValues[3].rows[I]    + EnsuredBoxValues[4].rows[I];
				   temp = temp + EnsuredBoxValues[2].columns[J] + EnsuredBoxValues[8].columns[J];
				   break;
			
			case 6: temp = temp + EnsuredBoxValues[7].rows[I]    + EnsuredBoxValues[8].rows[I];
				   temp = temp + EnsuredBoxValues[0].columns[J] + EnsuredBoxValues[3].columns[J];
				   break;
			
			case 7: temp = temp + EnsuredBoxValues[6].rows[I]    + EnsuredBoxValues[8].rows[I];
				   temp = temp + EnsuredBoxValues[1].columns[J] + EnsuredBoxValues[4].columns[J];
				   break;
				   
			case 8: temp = temp + EnsuredBoxValues[6].rows[I]    + EnsuredBoxValues[7].rows[I];
				   temp = temp + EnsuredBoxValues[2].columns[J] + EnsuredBoxValues[5].columns[J];
				   break;
		}
		return temp;
	}
	...
	if(type==1)
		{
			performGuarnteedRowColumnValueAnalysis();
			...
			for(int i=0;i<N;i++)
			{
				for(int j=0;j<N;j++)
				{
					if(cells[i][j].value==0)
					{
						Set temp=Set();
						temp=cells[i][j].PossibleValues;
						cells[i][j].PossibleValues= temp - boxes[getBoxValue(i,j)] - rows[i] - columns[j] - ensuredValues(i,j);
					}
				}
			}
		}
	...
	if(remainingnow==remainingCells)
				{
					//cout<<"Here!\n";
					break;
				}
				...
				{
					//cout<<"Calling filter(1) here\n";
					remainingnow = remainingCells;
					filter(1);
					continue;
				}
	void display()
